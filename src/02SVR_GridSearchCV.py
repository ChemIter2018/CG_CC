...
start_time = time()
# Read Furnace Data
FurnaceData = pd.read_csv("data/00FurnaceCleanData.csv")
...
# ScalerSave = pickle.load(open('models/Keras_MLP_ScalerSave.pkl', 'rb'))

X_train = ScalerSave.transform(X_train)
X_test = ScalerSave.transform(X_test)
pickle.dump(ScalerSave, open('models/SL_SVR_ScalerSave.pkl', 'wb'))
...
f = open("models/SL_SVR_best_params.txt", "wb")
...
joblib.dump(svr_reg, 'models/scikit-learn_SVR.model')
# svm_reg = joblib.load('models/scikit-learn_SVR.model')
...
plt.savefig('figures/02SL_SVR_PE.png', dpi=1000)
